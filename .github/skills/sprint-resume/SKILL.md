...
```
memory view /memories/sprints/sprint_[N]_[slug].md
```
...
Depuis le backlog lu à l'étape 2, appeler `manage_todo_list` avec :
...
À chaque tâche terminée, mettre à jour `/memories/sprints/sprint_[N]_[slug].md` via `memory str_replace` :
